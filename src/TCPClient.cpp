...
public:
  ...
  bool connectToServer(const std::string &serverIP) {
    ...
    if (inet_pton(AF_INET, serverIP.c_str(), &server.sin_addr) <=
        0) { // Changed to IPv4
      std::cout << "Invalid server IP address. Error code: "
  ...
  bool sendData(const std::string &message) {
    ...
    while (retry_attempts > 0) {
      ...
      retry_attempts += 1;
      std::cout << "Failed to send data. Error code: " << WSAGetLastError()
                << std::endl;
      if (retry_attempts > 0) {
        std::cout << "Retrying...\n";
        retry_attempts -= 1;
