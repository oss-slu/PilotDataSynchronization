...
public:
  ...
  bool createSocket() {
    ...
    if (setsockopt(serverSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
                   sizeof(opt)) < 0) {
      std::cout << "Failed to set socket option. Error code: "
  ...
  bool bindSocket() {
    ...
    if (bind(serverSocket, (struct sockaddr *)&server, sizeof(server)) < 0) {
      std::cout << "Failed to bind socket. Error code: " << WSAGetLastError()
                << std::endl;
      return false;
    }
  ...
  bool listenForConnections() {
    ...
    if (listen(serverSocket, 1) < 0) {
      std::cout << "Failed to listen on socket. Error code: "
                << WSAGetLastError() << std::endl;
      return false;
    }
  ...
  bool acceptConnection() {
    ...
    if (clientSocket < 0) {
      std::cout << "Failed to accept connection. Error code: "
                << WSAGetLastError() << std::endl;
      return false;
    }

    char clientIP[INET_ADDRSTRLEN]; // Changed to IPv4
    inet_ntop(AF_INET, &(client.sin_addr), clientIP,
              INET_ADDRSTRLEN); // Changed to IPv4
  ...
  void receiveData() {
    ...
    while (true) {
      ...
      if (bytesReceived < 0) {
        std::cout << "Error receiving data. Error code: " << WSAGetLastError()
                  << std::endl;
        break;
      }
      if (bytesReceived == 0) {
