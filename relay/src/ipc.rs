...
use crate::FromIpcThreadMessage;
...
pub(crate) fn ipc_connection_loop(
    ...
    txx: std::sync::mpsc::Sender<FromIpcThreadMessage>,
) {
            ...
            match conn.read_line(&mut buffer) {
                ...
                Ok(s) => {
                    ...
                    if buffer.starts_with("SHUTDOWN") {
                        let _ = txx.send(FromIpcThreadMessage::BatonShutdown);
                        break;
                    } else {
                        // actual baton data received
                        let _ = txx.send(FromIpcThreadMessage::BatonData(buffer.clone()));
                    }
